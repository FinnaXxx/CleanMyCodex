...
- 「静置」按**整棵子树里最新的修改时间**判断，不是目录自己的时间戳。目录的 mtime 只在它的直接子项
  增删时才变，所以正在往 `staging-x/plugins/browser/` 里解包的升级，`staging-x` 本身可能显示几小时没动过。
...
`~/.codex/.tmp` 的暂存目录和日志数据库压缩需要 Codex 完全退出。确认框会列出
被挡住的项目，并给两条路：

- **勾选「先退出 Codex，清理完成后自动重新打开」**：像按 ⌘Q 一样请求退出（不强杀），等它真的退出，
  执行清理，然后把刚才关掉的 App 重新打开。退不掉（比如有未保存内容）就中止清理并说明原因——
  不会在不安全的状态下硬来。
- **不勾选直接继续**：其余项目照常清理，这几项推迟到下一次。

终端里的 `codex` 进程**永远不会被自动结束**——它可能正在执行任务，而且没有办法请求一个终端进程
保存后退出。这种情况确认框只会告诉你有几个进程在跑，请你自己处理。

**会话瘦身不属于这一类。** 一个活着的会话只往它自己的 rollout 里追加，不碰别人的，所以判断依据
是「**这个文件**是不是正开着」，而不是「有没有 Codex 在跑」——用 `lsof` 查单个文件，代价很小。
于是终端里挂着一个会话，剩下一百多个照样能瘦身；正在被写入的那个自动跳过并说明是谁在用它。
`lsof` 不可用时退回到粗粒度判断：Codex 在跑就跳过。日志数据库压缩同样先查 `lsof`，
查不到才退回粗粒度规则。
...
- **「文稿」文件夹访问** —— 只有在你打开「工作产出」界面时才需要，因为要读 `~/Documents/Codex`。
  日常扫描**完全不碰** `~/Documents`，所以不打开那个界面就不会被问。
- **控制 Codex（自动化）** —— 只有在你勾选「先退出 Codex，清理完成后自动重新打开」时才需要。

### 为什么每次重新编译都要重新授权

macOS 的 TCC 按**代码签名身份**记录授权。ad-hoc 签名（`codesign -s -`）没有证书，身份退化成
二进制的 cdhash——每次重新编译都会变，系统就当成一个从没见过的新 App，于是所有权限重新问一遍。
...
它会在登录钥匙串里创建一个自签名的代码签名证书（不需要 Apple 开发者账号），
`build-app.sh` 之后会自动认出并使用它。身份稳定了，授权就能跨重新编译保留。
也可以用 `CODESIGN_IDENTITY="你的证书名" ./scripts/build-app.sh` 指定别的身份。

Gatekeeper 仍然不认识自签名证书，所以**第一次**打开需要右键 → 打开——这和权限提示是两回事。
