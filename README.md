...
- **`~/Library/Logs/com.openai.codex/YYYY/MM/DD`** — the desktop application's own logs, one file per session per process. A whole past day ages out together once nothing in it has been touched for ten days; the current day keeps every file still being written to. The year and month levels are never targets themselves, so a day that has come of age is not held back by a newer one beside it.
