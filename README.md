...
[Website](https://starbits.cc/clean-my-codex) · [Download](https://github.com/FinnaXxx/CleanMyCodex/releases)
...
- **Worktrees, taken down by git.** Codex checks a repository out under `~/.codex/worktrees` for each conversation that needs one, and a checkout with its dependencies installed runs to hundreds of megabytes. They are listed with their repository state, how much of them is build output, and how many conversations ran there. Removing one goes through `git worktree remove`, so the repository they came from stops listing a worktree that is no longer on disk.
- **Scheduled cleanup.** Runs on an interval over stale temporary folders, confirmed old plugin versions and aged-out conversations only. It skips pinned conversations, unfinished goals and queued work, and never touches caches, configuration, standalone session assets, worktrees or the workspace.
...
- **`generated_images/<thread-id>`** — standalone image directories produced by a conversation and listed on the Session Assets page.
- **`visualizations/YYYY/MM/DD/<thread-id>`** — the rich visual results Codex generates, such as JPG/PNG comparisons or HTML visualization previews. The date levels are walked recursively, listed as session assets and attributed to their source conversation.
- **`plans/<thread-id>`** — the plan documents Codex's plan mode produces, grouped by conversation. A conversation's directory may hold several revisions, which the scan collapses into one item named by the H1 of its newest revision; it is removed along with the conversation that owned it.
...
The current release does not scan, count, deduplicate or rewrite images embedded directly in conversation content; it manages only the ImageGen, Visualization and Plan directories Codex writes to disk, treating each Viewer as part of its Visualization. Session assets can be deleted on their own page, while deleting a conversation still removes every asset linked to that conversation and its subagents.
...
Configuration, credentials, the current plugins and the workspace are not removed along with a session; the `session_index.jsonl` lines tied directly to the target session are.
