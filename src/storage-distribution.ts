...
export type StorageDistributionKind = 'workspace' | 'sessions' | 'generatedAssets' | StorageSection | 'other'
...
export function storageDistribution(snapshot: ScanSnapshot): StorageDistribution {
  ...
  const classifiedCodexBytes = sessions + generatedAssets + sections.reduce((sum, item) => sum + item.bytes, 0)
