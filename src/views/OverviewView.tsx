          ...
          <div className="usage-distribution">
            <div className="distribution-bar" role="list" aria-label={t('空间占用分布', 'Storage distribution')}>
              {distribution.map((item, index) => {
                const fraction = item.bytes / Math.max(snapshot.totalCodexBytes, 1)
                const summary = `${formatBytes(item.bytes)} · ${distributionPercent.format(fraction)}`
                const description = [item.label, summary, ...(item.details ?? [])].join('；')
                return <span
                  key={item.label}
