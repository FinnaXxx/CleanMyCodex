...
export default function AutomationView() {
  ...
  return <>
    <section className="page-heading"><div><h2>自动清理</h2><p>由系统任务调度器定期运行。</p></div><span className={`pill ${state.loaded ? 'status-current' : ''}`}>{settings.enabled ? state.loaded ? '已安装' : '未加载' : '已关闭'}</span></section>
    {!state.supported && <p className="notice warning">定时后台清理目前支持 macOS 和 Windows。</p>}
    <section className="card form-card"><label className="strong"><input type="checkbox" checked={settings.enabled} onChange={(event) => update('enabled', event.target.checked)}/> 定期自动清理</label><label>每 <input className="number" type="number" min="1" max="180" value={settings.intervalDays} onChange={(event) => update('intervalDays', Number(event.target.value))}/> 天运行一次</label>{state.nextRunAt && <small>预计下次运行：{new Date(state.nextRunAt).toLocaleString()}</small>}{state.lastRun && <small>上次运行：{new Date(state.lastRun.finishedAt).toLocaleString()} · 释放 {formatBytes(state.lastRun.freedBytes)} · 成功 {state.lastRun.succeeded} 项 · 失败 {state.lastRun.failed} 项</small>}</section>
