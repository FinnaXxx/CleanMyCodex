...
export default function AutomationView() {
  ...
  return <>
    ...
    <section className="card form-card"><label className="strong"><input type="checkbox" checked={settings.enabled} onChange={(event) => update('enabled', event.target.checked)}/> 定期自动清理</label><label>每 <input className="number" type="number" min="1" max="180" value={settings.intervalDays} onChange={(event) => update('intervalDays', Number(event.target.value))}/> 天运行一次</label>{state.nextRunAt && <small>预计下次运行：{formatMoment(state.nextRunAt)}</small>}{state.lastRun && <small>上次运行：{formatMoment(state.lastRun.finishedAt)} · 释放 {formatBytes(state.lastRun.freedBytes)} · 成功 {state.lastRun.succeeded} 项 · 失败 {state.lastRun.failed} 项</small>}</section>
