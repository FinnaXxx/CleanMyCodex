...
export default function AutomationView({ onBack }: { onBack: () => void }) {
  ...
  return <div className="detail-content">
    ...
    <section className="card form-card"><h3>{t('定时清理范围', 'Cleanup Scope')}</h3>
      <label><input type="checkbox" checked={settings.cleanCaches} onChange={(event) => update('cleanCaches', event.target.checked)}/> {t('缓存、日志数据库和过期临时文件', 'Caches, log databases, and stale temporary files')}</label>
