...
export default function AutomationView() {
  ...
  return <>
    <section className="page-heading"><div><h2>自动清理</h2><p>通过 macOS LaunchAgent 定期运行；需要独占文件的项目会自动推迟。</p></div><span className={`pill ${state.loaded ? 'status-current' : ''}`}>{settings.enabled ? state.loaded ? '已安装' : '未加载' : '已关闭'}</span></section>
    {!state.supported && <p className="notice warning">定时后台清理目前只支持 macOS。</p>}
    ...
    <div className="page-footer"><span>{message ?? '修改后保存才会更新 LaunchAgent。'}</span><button className="clean" disabled={saving || (settings.enabled && !state.supported)} onClick={save}>{saving ? '保存中…' : '保存设置'}</button></div>
  </>
}
