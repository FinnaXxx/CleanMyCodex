...
function CleanupDialog({ preview, restart, forceQuit, onRestart, onForceQuit, onConfirm, onClose }: {
  ...
  return <div className="modal-backdrop"><section className="cleanup-dialog" role="dialog" aria-modal="true">
    <><h2>{t(`确认清理 ${preview.items.length} 项`, `Confirm cleanup of ${preview.items.length} items`)}</h2>
      ...
      <ul className="preview-list">{preview.items.map((item) => <li key={item.id}><span><strong>{item.title} <em className="method-badge">{language === 'zh-CN' ? CleanupMethodLabel[item.method] : ({ trash: 'Move to Trash', compactDatabase: 'Compact database' } as const)[item.method]}</em></strong><small>{item.detail}</small></span><b>{formatBytes(item.expectedBytes)}</b></li>)}</ul>
      {preview.warnings.map((warning) => <p className="notice warning" key={warning}>{warning}</p>)}
      {!!preview.blockedTitles.length && <div className="notice warning"><strong>{t(`${preview.blockedTitles.length} 项需要 Codex 完全退出`, `${preview.blockedTitles.length} items require Codex to quit completely`)}</strong><br/>{preview.blockedTitles.slice(0, 4).join(t('、', ', '))}
