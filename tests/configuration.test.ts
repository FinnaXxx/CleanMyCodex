...
`)
  ...
  it('resolves relative paths and protects their parents from cleanup', () => {
    ...
    const locations = new CodexLocations({ home, library: join(root, 'Library'), caches: join(root, 'Caches'), documents: join(root, 'Documents') })
    ...
    expect(() => guard.validate(source)).toThrow(/guard.protectedPath/)
  })
})
