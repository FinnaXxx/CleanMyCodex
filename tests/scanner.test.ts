...
describe('storage scanner semantics', () => {
  it('keeps recent logs, protects live marketplaces, and associates assets with their thread', async () => {
    ...
    expect(browserCache).toMatchObject({ group: 'review', risk: 'rebuildable' })
    ...
    expect(appCache).toMatchObject({ group: 'review', risk: 'rebuildable' })
    expect(appCache?.entries.some((entry) => entry.url === locations.codexCache)).toBe(true)
